 ...
 */
__kernel void shfl_up_sync(unsigned mask, unsigned var, unsigned int delta, int width,
    __local unsigned* shfl_up_sync_shared_var_arr, __local unsigned* shfl_up_sync_updated, __local unsigned* n) {
...
__kernel void test_shfl_up_sync_custom(__local unsigned* shfl_up_sync_shared_var_arr, __local unsigned* shfl_up_sync_updated) {
    ...
    for (int i=1; i<=4; i*=2) {
        ...
        shfl_up_sync(0xffffffff, value, i, 8, shfl_up_sync_shared_var_arr, shfl_up_sync_updated, &n);
        if ((laneId & 7) >= i)
            value += n;
    }
}